...
func newHostsMux(ff *util.FeatureFlags) *http.ServeMux {
	mux := http.NewServeMux()

	mux.Handle("/", hostsListEndpoint(ff))

	return mux
}

func hostsListEndpoint(ff *util.FeatureFlags) http.Handler {

	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Parse the index.html and login.partial.html templates from the embedded filesystem.
		tmpl, err := template.ParseFS(f, "index.html", "nav.partial.html", "hosts/list.partial.html")
		if err != nil {
